...
def stack_build(image, size, ra, dec, intense, wcs, scale, save, maxnoise, 
	subtract, fakesource):
	"""
	...
	@type wcs:			AstWCS.WCS
	@param wcs:			WCS for the source image
	...
	if scale != None:
		...
		if maxnoise != None:
			...
			fitswcs=wcs, maxnoise=maxnoise, savewcs=save)
			stamp.regrid(scale)
		else:
			stamp = Stampy(size+2, image, (ra, dec), name=name_string(ra, dec), 
			fitswcs=wcs, savewcs=save)
			stamp.regrid(scale)
	else:
		#This check allows for noise filtering to be turned off
		if maxnoise != None:
	...
	if fakesource != None and stamp.header['flag']['error_flag'] == 'a':
...
def headerize(image):
	...
	image[0].header.update('STACKMIN', Stack.percentile_centralpix(0), 
							'Minimum value of central pixel')
	image[0].header.update('STACKMAX', Stack.percentile_centralpix(100), 
							'Maximum value of central pixel')
	image[0].header.update('STACK50', Stack.percentile_centralpix(50), 
							'Median value of central pixel')
	image[0].header.update('STACK25', Stack.percentile_centralpix(25), 
							'25th Percentile value of central pixel')
	image[0].header.update('STACK75', Stack.percentile_centralpix(75), 
							'75th percentile value of central pixel')
	image[0].header.update('STACKNUM', Stack.data.shape[2], 
	...
	image[0].header.update('CRVAL1', -((image[0].data.shape[0]) / 2.0) * 
							image[0].header['CDELT1'], '')
	image[0].header.update('CRVAL2', -((image[0].data.shape[1]) / 2.0) * 
...
def save_stacks(name):
	...
	"""
	meanf = headerize(pyfits.HDUList(pyfits.PrimaryHDU(Stack.mean())))
	meanf.writeto(name + "_mean.fits")
	medianf = headerize(pyfits.HDUList(pyfits.PrimaryHDU(Stack.median())))
...
def save_percentile(name, percent):
	...
	"""
	percentf = headerize(pyfits.HDUList(pyfits.PrimaryHDU( \
...
def save_noise(file, ann, stamp, intensity, reject):
	...
	# NaN, we set it to 0
	try:
		cp = stamp.data[stamp.data.shape[0] / 2][stamp.data.shape[1] / 2]
		if isnan(cp):
			cp = 0
	except IndexError:
		cp = 0
	...
	for arg in imglist:
		...
		try:
			...
			try:
				...
				cdelt1 = wcs.getXPixelSizeDeg()
				cdelt2 = wcs.getYPixelSizeDeg()
			#minmax = wcs.getImageMinMaxWCSCoords()
			centre = wcs.getCentreWCSCoords()
			halfdims = wcs.getHalfSizeDeg()
			...
			for i in range(len(RA)):
				...
				Dec[i] < minmax[2] or Dec[i] > minmax[3]:
					...
					round(float(args[-1])/ 2.0), RA[i], Dec[i], Intense[i], wcs,
